...
class Auth extends \CodeIgniter\Controller
{
	...
	public function create_user()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run() && $this->ionAuth->register($identity, $password, $email, $additionalData)) {
			...
			$this->rander($this->renderPage($this->viewsFolder . DIRECTORY_SEPARATOR . 'create_user', $this->data));
	...
	public function rander($html)
	{
		...
		$view = 'Modules\dashboard\Views';
		echo view($view . '\index', $data);
	}
	...
	public function edit_user(int $id)
	{
		...
		$this->rander($this->renderPage($this->viewsFolder . DIRECTORY_SEPARATOR . 'edit_user', $this->data));
